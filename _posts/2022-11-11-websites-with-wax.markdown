...
date: 2022-11-11
---
One of the reasons I was keen to develop a blog using Jekyll, was in order to learn more about Wax. Wax is a framework for Jekyll that can be used make a configurable, static website with only a folder of images and a spreadsheet of image metadata. 
<!--more-->
As with Jekyll-based blogs, websites made with Wax are free to set up and host using GitHub pages, offer flexibility, freedom from ads and branding, and are particularly suitable for hosting high-quality image collections and scholarly exhibits. It's possible to embed pre-existing IIIF content in iframes, as on any other website, but Wax can also be used to create and serve IIIF-compliant images. Standard tifs and jpegs can be added to the site, transformed into IIIF tiles, compiled into multi-page objects, and examined in an OpenSeaDragon viewer. This allows anyone to host a free website featuring IIIF content, with no underlying infrastructure.
...
The images can be filtered (into photos, poems, or letters) via the homepage, or via the Browse page. The metadata attached to the images can be keyword searched via the Search page - results update responsively as keywords are entered. An Exhibits collection holds sample pages featuring illustrated narratives in markdown. These support html iframes - allowing the inclusion of pre-existing IIIF content, as well as the IIIF-images generated by Wax. One example demonstrates text wrapped around a Universal Viewer, embedded in an iframe, while the other uses an inline parallax image - a jpeg tiled by Wax for IIIF delivery. 

The results look very professional, and I've learned a lot of fundamental skills in the process. Wax offers a means of learning the basics of web development, data management, and plain text editing. I'm continuing to experiment with configuring its appearance, and researching options for supporting bilingual content.
...
* I used 'pid', 'label', 'author', 'description', '_date', and 'object_type' to add a tag/category allowing the objects to be filtered (see table below).
* Export the metadata file in csv, saving locally it with a simple name, e.g. metadata.csv
